...
const CurrencyConverterModal: React.FC<{ onClose: () => void; initialBalance: number }> = ({ onClose, initialBalance }) => {
    const initial = knutsToCanonical(initialBalance);
    const [galleons, setGalleons] = useState(initialBalance > 0 ? String(initial.galleons) : '');
    const [sickles, setSickles] = useState(initialBalance > 0 ? String(initial.sickles) : '');
    const [knuts, setKnuts] = useState(initialBalance > 0 ? String(initial.knuts) : '');
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <div className="container mx-auto p-4 pt-28 md:pt-32 pb-24">
      ...
      {showCurrencyModal && <CurrencyConverterModal onClose={() => setShowCurrencyModal(false)} initialBalance={currentUser.balance} />}
    </div>
  );
};
